...
export const AdminResume: React.FC = () => {
  const [resumeUrl, setResumeUrl] = useState("https://example.com/my-resume.pdf");
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [editUrl, setEditUrl] = useState("");

  const handleEdit = () => {
    setEditUrl(resumeUrl);
    setIsModalOpen(true);
  };

  const handleSave = () => {
    setResumeUrl(editUrl);
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-sm text-gray-500">Update your CV link visible to recruiters.</p>
        </div>
        {!resumeUrl && <Button onClick={() => setIsModalOpen(true)}>+ Add Resume</Button>}
      </div>
      ...
      {resumeUrl ? (
        <Card className="flex flex-col md:flex-row items-center justify-between gap-6 hover:bg-white/[0.02] transition-colors">
          <div className="flex items-center space-x-6">
            ...
            <div>
              ...
              >
                {resumeUrl} <ExternalLink size={12} />
              </a>
              <p className="text-xs text-gray-500 mt-2">Uploaded: Oct 24, 2023</p>
            </div>
          </div>
          
          <div className="flex items-center gap-3 w-full md:w-auto">
            <Button variant="secondary" onClick={() => window.open(resumeUrl, '_blank')}>View</Button>
            <Button variant="primary" onClick={handleEdit}><Edit2 size={16} className="mr-2"/> Update</Button>
        ...
        <div className="text-center py-20 border-2 border-dashed border-white/10 rounded-2xl bg-white/5 flex flex-col items-center justify-center">
          ...
          <p className="text-gray-500 mb-6">Upload a PDF or provide a link to your resume.</p>
          <Button onClick={() => setIsModalOpen(true)}>Upload Resume</Button>
        </div>
      )}

      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Update Resume URL">
