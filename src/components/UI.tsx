...
export const Card: React.FC<{ children: React.ReactNode; className?: string }> = ({ children, className = '' }) => (
  <div className={`bg-slate-900/40 backdrop-blur-xl border border-white/5 rounded-2xl shadow-xl p-6 ${className}`}>
    {children}
  </div>
);
...
export const Button: React.FC<ButtonProps> = ({ children, variant = 'primary', size = 'md', className = '', ...props }) => {
  ...
  const variants = {
    primary: "bg-cyan-500/10 text-cyan-400 border border-cyan-500/20 hover:bg-cyan-500/20 hover:shadow-[0_0_15px_rgba(34,211,238,0.1)]",
...
export const Input: React.FC<InputProps> = ({ label, className = '', ...props }) => (
  <div className="mb-4">
    {label && <label className="block text-xs font-medium text-gray-400 uppercase tracking-wider mb-2">{label}</label>}
    <input
...
// --- Badge ---
export const Badge: React.FC<{ children: React.ReactNode; color?: 'cyan' | 'green' | 'yellow' | 'purple' }> = ({ children, color = 'cyan' }) => {
  const colors = {
